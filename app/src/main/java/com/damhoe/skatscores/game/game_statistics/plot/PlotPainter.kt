...
import com.damhoe.skatscores.game.game_statistics.presentation.GraphicUtils
...
class PlotPainter {
    ...
    fun drawGrid(
        ...
        val (exponent, realTicks) = calculateTickPositions(bottom, top, ratio)
        ...
        for ((realY, y) in realTicks.zip(ticks)) {
            mCanvas.drawLine(pxLeft, y, pxRight, y, gridPaint)
    ...
    private val minTickDistance: Float = GraphicUtils.dpToPx(50f).toFloat()
     ...
     * interval (0.1, 1.0]
     */
    private fun calculateTickPositions(min: Float, max: Float, ratio: Float): Pair<Int, List<Double>> {
        ...
        val increment = roundUpToNearest(realMinDist.toDouble(), minTickIncrement)
        val firstTick = roundUpToNearest(min.toDouble(), increment)
    ...
    private fun roundUpToNearest(value: Double, increment: Double): Double =
        if (increment == 0.0) {
            value
        } else {
    ...
    private fun drawMarkers(data: PlotData, paint: Paint) {
        ...
        for (point: PointF in data.points.filter { mTransform.isVisible(it) }) {
            mCanvas.drawCircle(point.x, point.y, dpToPx(mStyle.markerSize).toFloat(), paint)
        }
    }
    ...
    fun drawLabels(data: List<PlotData>) {
        ...
        for (line in data) {
            ...
            mCanvas.drawRoundRect(
                x, bottom - markerSize, x + markerSize, bottom,
                cornerRadius, cornerRadius, legendMarkerPaint
            )

            x += markerSize + gap
            mCanvas.drawText(line.label, x, bottom, legendPaint)
